...
export class UsersApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param offset Offset
     */
    public async v1UsersGet(limit?: number, offset?: number, _options?: Configuration): Promise<RequestContext> {
...
export class UsersApiResponseProcessor {
     ...
     */
     public async v1UsersGetWithHttpInfo(response: ResponseContext): Promise<HttpInfo<ModelsUserList >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: ModelsUserList = ObjectSerializer.deserialize(
        ...
        if (isCodeInRange("500", response.httpStatusCode)) {
            ...
            throw new ApiException<string>(response.httpStatusCode, "Error listing users", body, response.headers);
        }
