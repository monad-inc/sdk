...
export class SessionsApiRequestFactory extends BaseAPIRequestFactory {
    ...
    public async createSession(createSessionRequest?: CreateSessionRequest, _options?: Configuration): Promise<RequestContext> {
        ...
        // Path Params
        const localVarPath = '/v3/sessions';
...
export class SessionsApiResponseProcessor {
     ...
     */
     public async createSessionWithHttpInfo(response: ResponseContext): Promise<HttpInfo<RoutesV3CreateSessionResponse >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: RoutesV3CreateSessionResponse = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "RoutesV3CreateSessionResponse", ""
            ) as RoutesV3CreateSessionResponse;
        ...
        if (isCodeInRange("401", response.httpStatusCode)) {
            ...
            throw new ApiException<string>(response.httpStatusCode, "Missing or invalid auth", body, response.headers);
        }
