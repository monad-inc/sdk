...
type ApiListTagResourcesRequest struct {
	ctx context.Context
	ApiService *TagsAPIService
	organizationId string
	tag string
...
func (r ApiListTagResourcesRequest) Limit(limit int32) ApiListTagResourcesRequest {
	r.limit = &limit
	return r
}

// Offset
func (r ApiListTagResourcesRequest) Offset(offset int32) ApiListTagResourcesRequest {
	r.offset = &offset
	return r
}

func (r ApiListTagResourcesRequest) Execute() (*RoutesV3TagResourcesResponse, *http.Response, error) {
	return r.ApiService.ListTagResourcesExecute(r)
}

/*
ListTagResources List a tag's resources
...
List the resources (v1: pipelines) a customer tag is attached to, by tag ID or name. Each resource carries a flat kind (pipeline, input, output, enrichment, transform). Reserved tags return 404.
 ...
 @param organizationId Organization ID
 @param tag Tag ID or name
 @return ApiListTagResourcesRequest
*/
func (a *TagsAPIService) ListTagResources(ctx context.Context, organizationId string, tag string) ApiListTagResourcesRequest {
	return ApiListTagResourcesRequest{
		ApiService: a,
		ctx: ctx,
		organizationId: organizationId,
...
func (a *TagsAPIService) ListTagResourcesExecute(r ApiListTagResourcesRequest) (*RoutesV3TagResourcesResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *RoutesV3TagResourcesResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "TagsAPIService.ListTagResources")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v3/{organization_id}/tags/{tag}/resources"
	localVarPath = strings.Replace(localVarPath, "{"+"organization_id"+"}", url.PathEscape(parameterValueToString(r.organizationId, "organizationId")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"tag"+"}", url.PathEscape(parameterValueToString(r.tag, "tag")), -1)
